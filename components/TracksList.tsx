...
function TracksList(props: ITracksProp) {
  const { playlistId } = props;

  const [loading, setLoading] = useState<boolean>(false);
  const [tracks, setTracks] = useState<Array<ITrack>>([]);

  const getColor = async (image: string) => (await fac.getColorAsync(image)).hex;

  useEffect(() => {
    const fetchData = async () => {
      if (playlistId) {
        const results = await fetchTracks(playlistId);
        setTracks(results);
        setLoading(false);
      }
    };
    ...
    <div className="bg-gray-100 mt-4 md:mt-2 overflow-hidden   md:rounded-lg  md:border ">
      ...
      >
        {tracks.map((track, index) => (
          <Track bgColor={getColor(track.image)} key={track.id} index={index} track={track} />
        ))}
        {/* {tracks.length < 5 &&
